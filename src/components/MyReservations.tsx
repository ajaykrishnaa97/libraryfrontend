...
const MyReservations: React.FC = () => {
    ...
    const fetchReservations = async () => {
        try {
            ...
            const response = await axios.get<MyReservation[]>('http://localhost:5134/books/my-reservations', {
                    ...
                    >
                        ...
                        <div className="flex-grow-1">
                            <div className="fw-semibold">
                                {res.bookTitle} <span className="text-muted small">by {res.bookAuthor}</span>
                            </div>
                            <div className="small text-muted mb-1">
                                📍 <strong>Copy:</strong> {res.bookCopyLocation} | 🏠 <strong>Return:</strong> {res.returnLocation}
                            </div>
                            <div className="small text-muted mb-1">
                                📅 <strong>Reserved:</strong> {new Date(res.reservedAt).toLocaleString()}
                            </div>
                            <div className="small text-muted">
                                ⏰ <strong>Expires:</strong> {new Date(res.expiresAt).toLocaleString()}
                            </div>
                        </div>

                        <span className={`badge ${res.status === 'Active' ? 'bg-success' : 'bg-secondary'} rounded-pill ms-3`}>
                            {res.status}
                        </span>
                    </div>
                ))
            ) : (
                <div className="text-center text-muted py-4">No active reservations.</div>
            )}
        </div>
    );
};
